...
(() => {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    // Fix: Ensure original functions are properly preserved by working with window versions
    const originalStartGame = window.startGame;

    // Enhance startGame function
    window.startGame = function(ctx, canvas) {
      try {
        ...
        showErrorModal("Start Game Error", "Failed to start the game: " + error.message);
      }
    };
    ...
    const originalHandleConflict = window.handleConflict;
    window.handleConflict = function() {
      try {
    ...
    const originalHandlePowerup = window.handlePowerup;
    window.handlePowerup = function() {
      try {
        gameState.score += 50;
        gameState.grid[gameState.playerY][gameState.playerX] = TILE_TYPES.EMPTY;
        updateUIElements();
    ...
    const originalHandleBug = window.handleBug;
    window.handleBug = function(message) {
      try {
        gameState.lives--;
    ...
    const originalHandleCommit = window.handleCommit;
    window.handleCommit = function(ctx, canvas) {
      try {
        gameState.gameRunning = false;
        gameState.score += 500;
    ...
    const startButton = document.getElementById('startButton');
    if (startButton) {
      console.log('Re-attaching start button event listener');
